...
class LiveChatController extends Controller {
    ...
    public function index() {
        ...
        $admins = Users::whereIn('role', ['admin', 'operator'])
    ...
    public function fetchMessages($usersId) {
        $superadminId = Auth::id();

        // Cari riwayat chat
        $messages = Message::where(function ($q) use ($superadminId, $usersId) {
            $q->where('sender_id', $superadminId)->where('receiver_id', $usersId);
        })->orWhere(function ($q) use ($superadminId, $usersId) {
            $q->where('sender_id', $usersId)->where('receiver_id', $superadminId);
        ...
        Message::where('sender_id', $usersId)
            ->where('receiver_id', $superadminId)
            ->where('is_read', false)
            ->update(['is_read' => true]);
