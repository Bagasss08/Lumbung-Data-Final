...
<style>
    ...
    .notif-check-btn:hover { border-color: #10b981; background: #ecfdf5; }
    .notif-check-btn svg { width: 11px; height: 11px; stroke: #9ca3af; transition: stroke 0.15s; }
    .notif-check-btn:hover svg { stroke: #10b981; }
    ...
    @media (max-width: 380px) {
        .nav-modal-card { padding: 1.5rem 1.25rem; }
    }
    ...
    .nav-option-card:hover .nav-icon-wrap { transform: scale(1.08); }
    .nav-icon-wrap {
...
<div
    ...
    role="dialog"
    aria-modal="true"
    aria-labelledby="nav-modal-login-title"
    aria-hidden="true"
>
    <div class="nav-modal-card" id="nav-login-modal-card">
        ...
        <div class="text-center mb-7">
            @if($identitas_nav && $identitas_nav->logo_desa && file_exists(storage_path('app/public/logo-desa/' . $identitas_nav->logo_desa)))
                <img src="{{ asset('storage/logo-desa/' . $identitas_nav->logo_desa) }}"
                     alt="Logo Desa"
        ...
        <div class="space-y-3">
            ...
            >
                <div class="nav-icon-wrap bg-emerald-500 shadow-md shadow-emerald-200">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-5.356-3.712M9 20H4v-2a4 4 0 015.356-3.712M15 11a4 4 0 11-8 0 4 4 0 018 0zm6 0a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                </div>
                ...
                <svg class="w-4 h-4 text-slate-300 group-hover:text-emerald-500 transition-colors flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            >
                ...
                <svg class="w-4 h-4 text-slate-300 group-hover:text-blue-500 transition-colors flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
                </svg>
            </a>
        ...
        <div class="flex items-center justify-between h-20">
            ...
            <div class="hidden xl:flex items-center justify-center gap-6 2xl:gap-8 h-full">
                ...
                <div class="relative group h-full">
                    <button class="flex items-center h-full gap-1 text-sm font-medium text-slate-600 group-hover:text-emerald-600 transition duration-300">
                        Profil Desa
                ...
                <div class="relative group h-full">
                    ...
                    <div class="absolute top-full left-0 w-56 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-[100] transform origin-top translate-y-2 group-hover:translate-y-0">
                        <div class="bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden py-2">
                            ...
                            <a href="{{ route('wisata') }}" class="group flex items-center gap-3 px-5 py-2.5 text-sm text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition">
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-emerald-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @auth
                    @if(Auth::user()->role == 'warga')
                        ...
                        <div x-data="wargaNotifApp()" x-init="init()" class="notif-bell-wrap">
                            ...
                            <button @click="toggleDropdown()" class="p-2 rounded-lg transition-all" :class="dropdownOpen ? 'bg-emerald-50 text-emerald-600' : 'text-slate-400 hover:text-emerald-600 hover:bg-emerald-50'">
                            ...
                            <div x-show="dropdownOpen"
                                ...
                                <div class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 flex-shrink-0">
                                    ...
                                    <span x-show="totalNotif > 0" x-text="totalNotif + ' baru'"
                                          class="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ml-2" style="display:none"></span>
                                </div>
                                ...
                                <div class="flex-1 overflow-y-auto divide-y divide-slate-50 min-h-0">
                                    ...
                                    <template x-for="item in notifItems" :key="item.id">
                                        <div class="notif-item-row flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors" :class="!item.dibaca ? 'bg-emerald-50/40' : ''">
                                            ...
                                            <a :href="item.url" class="flex-1 min-w-0 block">
                                                <p class="text-xs font-semibold text-slate-700 truncate" x-text="item.title"></p>
                                            ...
                                            <div x-show="!item.dibaca" class="notif-item-right">
                                                <button @click.stop="markOneRead(item, $event)" class="notif-check-btn" title="Tandai sudah dibaca">
                                ...
                                <div class="px-4 py-2.5 bg-slate-50 border-t border-slate-100 flex justify-center items-center flex-shrink-0">
                                    <a href="{{ route('warga.notifikasi.index') }}" class="text-xs text-emerald-600 font-semibold hover:underline flex items-center gap-1 whitespace-nowrap">
                                        Selengkapnya...
                    ...
                    <div class="relative group z-50">
                        ...
                        <div class="absolute top-full right-0 pt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-right z-[100]">
                            <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-2">
                                @if(Auth::user()->role == 'warga')
                                    ...
                                    <a href="{{ route('warga.aktivitas') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-slate-600 rounded-xl hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
                                ...
                                <form action="{{ route('logout') }}" method="POST">
                                    ...
                                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-red-600 rounded-xl hover:bg-red-50 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/></svg>
                                        Keluar
                                    </button>
                                </form>
                            </div>
        ...
        <div id="mobile-menu" class="hidden xl:hidden border-t border-slate-100 py-4 space-y-2 animate-fade-in-down bg-white max-h-[80vh] overflow-y-auto">
            ...
            <details class="group [&::-webkit-details-marker]:hidden">
                <summary class="flex items-center justify-between px-4 py-3 rounded-xl font-medium text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 cursor-pointer list-none transition">
                    Profil Desa
                    <svg class="w-5 h-5 transition-transform group-open:rotate-180 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </summary>
            ...
            <details class="group [&::-webkit-details-marker]:hidden">
                ...
                <div class="px-3 py-2 ml-2 border-l-2 border-emerald-100 space-y-1 mt-1">
                    ...
                    <a href="{{ route('wisata') }}" class="flex items-center gap-3 px-3 py-2.5 text-sm text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">Wisata</a>
                </div>
            </details>
            ...
            <div class="pt-4 mt-2 border-t border-slate-100 px-2 space-y-3">
                ...
                @auth
                    ...
                    @if(Auth::user()->role == 'warga')
                        ...
                        <a href="{{ route('warga.aktivitas') }}" class="flex items-center gap-3 px-4 py-3 text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition font-medium">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="mt-2 pt-2 border-t border-slate-100">
                        ...
                        <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-50 text-red-600 font-semibold rounded-2xl hover:bg-red-100 transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/></svg>
                            Keluar
                        </button>
                    </form>
                @endauth
...
<script>
    ...
    (function () {
        ...
        function openLoginModal() {
            ...
            setTimeout(() => { if (btnClose) btnClose.focus(); }, 50);
        }
    ...
    function wargaNotifApp() {
        return {
            ...
            init() {
                ...
                this._fetchBadges(false).then(() => { this._initialized = true; });
            ...
            async toggleDropdown() {
                ...
                if (this.dropdownOpen) await this._fetchList();
            },

            async _fetchBadges(playSound) {
                try {
                    const res  = await fetch('/warga/notifikasi/badges', { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
            ...
            async _fetchList() {
                this.loading = true;
                try {
                    const res  = await fetch('/warga/notifikasi/list', { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
                    if (!res.ok) throw new Error('HTTP ' + res.status);
                    const data = await res.json();
            ...
            async markOneRead(item, event) {
                ...
                try {
                    ...
                    setTimeout(() => {
                        item.dibaca = true;
                        if (this.totalNotif > 0) { this.totalNotif--; this._prevTotal = this.totalNotif; }
                    }, 300);
                } catch (e) {
            ...
            _playSound() {
                if (!this.soundEnabled || this._audioPlaying) return;
                this._audioPlaying = true;
                try {
                    const snd = new Audio('/sounds/notif.mp3');
                    snd.volume = 0.6;
                    snd.play()
                        .then(() => { snd.addEventListener('ended', () => { this._audioPlaying = false; }, { once: true }); })
                        .catch(() => { this._audioPlaying = false; this._playBeep(); });
                } catch (e) { this._audioPlaying = false; }
            },
            ...
            _playBeep() {
                ...
                try {
                    ...
                    osc.addEventListener('ended', () => { ctx.close(); this._audioPlaying = false; });
                } catch (e) { this._audioPlaying = false; }
            },
        };
    }
