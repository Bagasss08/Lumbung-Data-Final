            ...
            : ($bantuan->sasaran == 3
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.bantuan.edit', $bantuan->id) }}"
                        class="inline-flex items-center gap-1.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors shadow-sm group">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.bantuan.index') }}"
                        class="inline-flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors shadow-sm group">
        ...
        <div class="p-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4">
                    <p class="text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-wide mb-1">Asal
                        Dana</p>
                    <p class="text-sm font-bold text-gray-800 dark:text-slate-200">{{ $bantuan->asal_dana ?? '-' }}</p>
                </div>
                <div class="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4">
                    <p class="text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-wide mb-1">Tahun
                    </p>
                    <p class="text-sm font-bold text-gray-800 dark:text-slate-200">{{ $bantuan->tahun ?? '-' }}</p>
                </div>
                <div class="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4">
                    <p class="text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-wide mb-1">Nominal
                    </p>
            ...
            @if ($bantuan->syarat)
                <div class="mb-4">
                    ...
                    <div class="bg-yellow-50 border border-yellow-100 rounded-xl p-4 text-sm text-gray-700">
                        {{ $bantuan->syarat }}</div>
                </div>
            @endif
    ...
    <div
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr
                        class="bg-gray-50 dark:bg-slate-700/50 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide dark:divide-slate-700">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($peserta as $i => $p)
                        <tr
                            class="hover:bg-gray-50/70 dark:hover:bg-slate-700/50 transition-colors duration-150 divide-gray-100 dark:divide-slate-700">
                            <td class="px-6 py-4 text-sm text-gray-400 dark:text-slate-200">
                                {{ $peserta->firstItem() + $i }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td colspan="6" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <a href="{{ route('admin.bantuan.peserta.create', $bantuan->id) }}"
                                        class="text-sm text-emerald-600 hover:text-emerald-700 font-medium">+ Tambah
                                        Peserta
                                        Pertama</a>
                                </div>
                            </td>
