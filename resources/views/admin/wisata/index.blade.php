...
<div x-data="{ showDeleteModal: false, deleteId: null }">
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-gray-500">Kelola data destinasi wisata desa</p>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-gray-400 font-medium">Total Wisata</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-teal-700">{{ $stats['aktif'] }}</p>
            </div>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-gray-500">{{ $stats['nonaktif'] }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
        <form method="GET" action="{{ route('admin.wisata.index') }}" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Nama wisata, lokasi..."
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide w-16">Foto</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Nama Wisata</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/70 transition-colors">
                        <td class="px-4 py-3.5 text-gray-400 text-xs">
                            {{ $loop->iteration + ($wisatas->currentPage() - 1) * $wisatas->perPage() }}
                        </td>
                        ...
                        <td class="px-4 py-3.5">
                            <span class="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-xs font-medium rounded-full">
                                {{ $item->kategori }}
                            </span>
                        </td>
                        <td class="px-4 py-3.5 text-gray-500 text-xs max-w-[160px]">
                    ...
                    <tr>
                        <td colspan="8" class="px-4 py-16 text-center">
                            ...
                            <p class="text-sm text-gray-400 font-medium">Belum ada data wisata</p>
                            <a href="{{ route('admin.wisata.create') }}"
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            <p class="text-xs text-gray-500">
                Menampilkan {{ $wisatas->firstItem() }}–{{ $wisatas->lastItem() }} dari {{ $wisatas->total() }} data
            </p>
            <div class="text-sm">{{ $wisatas->links() }}</div>
        </div>
        @endif
    </div>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4"
            ...
            <h3 class="text-base font-bold text-gray-900 text-center mb-1">Hapus Wisata?</h3>
            <p class="text-sm text-gray-500 text-center mb-6">Data wisata ini akan dihapus permanen dan tidak dapat dikembalikan.</p>
