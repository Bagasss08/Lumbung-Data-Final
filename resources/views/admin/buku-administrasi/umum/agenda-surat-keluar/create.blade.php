...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Tambah Surat Keluar</h2>
        <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Catat agenda surat keluar baru ke dalam buku administrasi</p>
    </div>
    <nav class="flex items-center gap-1.5 text-sm">
        ...
        <a href="{{ route('admin.buku-administrasi.umum.agenda-surat-keluar.index') }}" class="text-gray-400 hover:text-emerald-600 transition-colors">Agenda Surat Keluar</a>
        <svg class="w-3.5 h-3.5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
    ...
    <div class="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-5">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-white font-semibold text-base">Formulir Surat Keluar</h2>
                <p class="text-emerald-100 text-xs mt-0.5">Lengkapi data agenda surat keluar desa</p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.buku-administrasi.umum.agenda-surat-keluar.store') }}" method="POST" class="p-6 space-y-8">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="tanggal_pengiriman" value="{{ old('tanggal_pengiriman') }}" required
                           class="w-full px-3 py-2.5 text-sm border @error('tanggal_pengiriman') border-red-400 @else border-gray-200 dark:border-slate-600 @enderror rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
                ...
                <div>
                    ...
                    <input type="text" name="nomor_surat" value="{{ old('nomor_surat') }}" required placeholder="Contoh: 001/DS/V/2026"
                ...
                <div>
                    ...
                    <input type="text" name="tujuan" value="{{ old('tujuan') }}" required placeholder="Nama instansi / orang yang dituju"
                           class="w-full px-3 py-2.5 text-sm border @error('tujuan') border-red-400 @else border-gray-200 dark:border-slate-600 @enderror rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-colors">
                    @error('tujuan')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>

        {{-- SECTION: Isi & Keterangan --}}
        <div>
            <div class="flex items-center gap-3 mb-4">
