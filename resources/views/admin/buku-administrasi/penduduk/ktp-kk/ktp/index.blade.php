...
@section('title', 'Data KTP / Penduduk')
...
@section('content')
    <div x-data>
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-lg font-bold text-gray-700 dark:text-slate-200">Data KTP / Penduduk</h2>
                <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Daftar data identitas penduduk</p>
            </div>
            <div class="flex items-center gap-1.5">
                <nav class="flex items-center gap-1.5 text-sm">
                    ...
                    <span class="text-gray-600 dark:text-slate-300 font-medium">KTP</span>
                </nav>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-2 mb-6">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 dark:text-slate-500">KTP / Penduduk</p>
                </div>
            </div>

            <a href="{{ route('admin.buku-administrasi.penduduk.ktp-kk.ktp.create') }}"
        ...
        {{-- FILTER                                                       --}}
        {{-- ============================================================ --}}
        <form method="GET" action="{{ route('admin.buku-administrasi.penduduk.ktp-kk.ktp.index') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                {{-- Status Hidup --}}
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1.5">Status Hidup</label>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.buku-administrasi.penduduk.ktp-kk.ktp.index') }}" title="Reset Filter"
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            {{ $ktp->jenis_kelamin == 'L'
                                ...
                                : 'bg-pink-100 dark:bg-pink-900/40 text-pink-700 dark:text-pink-300' }}">
                                            {{ $ktp->jenis_kelamin === 'L' ? 'L' : 'P' }}
                                        </span>
                                    </td>

                                    {{-- Tempat & Tanggal Lahir --}}
                                    <td
                                        class="px-4 py-4 text-gray-600 dark:text-slate-400 hidden md:table-cell whitespace-nowrap text-xs">
                                        <span>{{ $ktp->tempat_lahir }},</span>
                                        <span class="block text-xs text-gray-400 dark:text-slate-500">
                                            {{ \Carbon\Carbon::parse($ktp->tanggal_lahir)->format('d/m/Y') }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-4 hidden lg:table-cell">
                                        @php
                                            $statusKawinColor = match ($ktp->status_kawin ?? '') {
                                                'Kawin' => 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300',
                                                'Belum Kawin' => 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400',
                                                'Cerai Hidup' => 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300',
                                                'Cerai Mati' => 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300',
                                                default => 'bg-gray-100 dark:bg-slate-700 text-gray-500',
                                            };
                                        @endphp
                                        @if ($ktp->status_kawin)
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $statusKawinColor }}">
                                                {{ $ktp->status_kawin }}
                                            </span>
                                        @else
                                ...
                                : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300' }}">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end gap-1.5">
                                            <a href="{{ route('admin.buku-administrasi.penduduk.ktp-kk.ktp.show', $ktp) }}"
                                                title="Lihat Detail"
                                            ...
                                            <button type="button"
                                                @click="$dispatch('buka-modal-hapus', { action: '{{ route('admin.buku-administrasi.penduduk.ktp-kk.ktp.destroy', $ktp) }}', nama: '{{ addslashes($ktp->nama) }}' })"
                                                title="Hapus"
