...
@section('content')
    <div class="flex items-center gap-2 mb-1">
        ...
        <p class="text-lg font-semibold text-gray-700">Edit Kartu Keluarga</p>
    </div>
    <p class="text-sm text-gray-400 mb-6 ml-7">No KK: {{ $kk->no_kk }}</p>
    ...
    <form action="{{ route('admin.buku-administrasi.penduduk.ktp-kk.kk.update', $kk) }}" method="POST">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="no_kk" value="{{ old('no_kk', $kk->no_kk) }}" maxlength="16"
                ...
                <div>
                    ...
                    <input type="date" name="tgl_terdaftar" value="{{ old('tgl_terdaftar', $kk->tgl_terdaftar?->format('Y-m-d')) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="alamat" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('alamat', $kk->alamat) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <select name="klasifikasi_ekonomi" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="{{ $k }}" @selected(old('klasifikasi_ekonomi', $kk->klasifikasi_ekonomi) === $k)>{{ ucwords(str_replace('_', ' ', $k)) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Bantuan Aktif</label>
                    <input type="text" name="jenis_bantuan_aktif" value="{{ old('jenis_bantuan_aktif', $kk->jenis_bantuan_aktif) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-4">
            ...
            <div id="anggota-container" class="space-y-3">
                @foreach($kk->anggota as $i => $p)
                <div class="border border-gray-200 rounded-lg p-4 relative anggota-item grid grid-cols-1 md:grid-cols-3 gap-4" id="anggota-{{ $i }}">
                    <button type="button" onclick="hapusAnggota({{ $i }})" class="absolute top-3 right-3 text-red-400 hover:text-red-600">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                    <div class="md:col-span-2">
                        <label class="block text-xs text-gray-600 mb-1">Penduduk *</label>
                        <select name="anggota[{{ $i }}][penduduk_id]" required
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
                            @foreach($pendudukList as $pd)
                            <option value="{{ $pd->id }}" @selected($p->id === $pd->id)>{{ $pd->nik }} — {{ $pd->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">Hubungan Keluarga *</label>
                        <select name="anggota[{{ $i }}][hubungan_keluarga]" required
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
                            @foreach(['kepala_keluarga','istri','anak','menantu','cucu','orang_tua','mertua','saudara','lainnya'] as $h)
                            <option value="{{ $h }}" @selected($p->pivot->hubungan_keluarga === $h)>{{ ucwords(str_replace('_', ' ', $h)) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <p id="no-anggota" style="{{ $kk->anggota->count() ? 'display:none' : '' }}"
               class="text-sm text-gray-400 text-center py-4">Belum ada anggota.</p>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-5 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors">Perbarui</button>
        </div>
    </form>
    ...
    let anggotaCount = {{ $kk->anggota->count() }};
    ...
    function buatFormAnggota(index) {
        ...
        <div class="border border-gray-200 rounded-lg p-4 relative anggota-item grid grid-cols-1 md:grid-cols-3 gap-4" id="anggota-${index}">
            <button type="button" onclick="hapusAnggota(${index})" class="absolute top-3 right-3 text-red-400 hover:text-red-600">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
            <div class="md:col-span-2">
                <label class="block text-xs text-gray-600 mb-1">Penduduk *</label>
