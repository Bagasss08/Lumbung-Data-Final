...
<div x-data="{ showDeleteModal: false, deleteId: null, deleteName: '' }">
    ...
    <div class="flex items-center justify-between mb-5">
        <div>
            <h2 class="text-lg font-bold text-gray-800 dark:text-slate-100">Data Lapak</h2>
            <p class="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Kelola data lapak & produk usaha warga desa</p>
        </div>
        <nav class="flex items-center gap-1.5 text-sm">
            ...
            <span class="text-gray-600 dark:text-slate-300 font-medium">Data Lapak</span>
        </nav>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700" style="overflow: visible">
        ...
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
            class="flex items-center gap-3 mx-5 mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 text-emerald-800 dark:text-emerald-300 rounded-xl">
        ...
        <div class="px-4 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
            <form method="GET" action="{{ route('admin.lapak.index') }}" id="form-filter-lapak"
                class="flex flex-wrap items-center gap-2">

                <input type="hidden" name="status" id="val-lapak-status" value="{{ request('status') }}">
                ...
                <div class="relative w-48" x-data="{
                    ...
                    choose(opt) {
                        ...
                        document.getElementById('val-lapak-status').value = opt.value;
                        this.open = false;
                        this.search = '';
                        document.getElementById('form-filter-lapak').submit();
                    },
                    reset() {
                        this.selected = '';
                        document.getElementById('val-lapak-status').value = '';
                        this.open = false;
                        this.search = '';
                        document.getElementById('form-filter-lapak').submit();
                    }
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 px-5 py-3 border-b border-gray-100 dark:border-slate-700">
            ...
            {{-- Tampilkan X entri --}}
            <form method="GET" action="{{ route('admin.lapak.index') }}" id="form-per-page-lapak"
                class="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-400">
                @foreach(request()->except('per_page', 'page') as $key => $val)
                    <input type="hidden" name="{{ $key }}" value="{{ $val }}">
                @endforeach
                <input type="hidden" name="per_page" id="val-per-page-lapak" value="{{ request('per_page', 10) }}">
                ...
                <div class="relative w-24" x-data="{
                    ...
                    choose(opt) {
                        this.selected = opt.value;
                        document.getElementById('val-per-page-lapak').value = opt.value;
                        this.open = false;
                        document.getElementById('form-per-page-lapak').submit();
                    }
                }" @click.away="open = false">
                    <button type="button" @click="open = !open"
            ...
            {{-- Search --}}
            <form method="GET" action="{{ route('admin.lapak.index') }}" class="flex items-center gap-2">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-3 py-3">
                            <div class="flex items-center gap-1 flex-nowrap">
                                ...
                                {{-- Detail (indigo) --}}
                                <a href="{{ route('admin.lapak.show', $item) }}" title="Detail Lapak"
                                ...
                                {{-- Produk (blue) --}}
                                <a href="{{ route('admin.lapak.produk.index', $item) }}" title="Kelola Produk"
                                    class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-blue-500 hover:bg-blue-600 text-white transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                {{-- Edit (amber) --}}
                                <a href="{{ route('admin.lapak.edit', $item) }}" title="Edit Lapak"
                                ...
                                <form action="{{ route('admin.lapak.toggle-status', $item) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <button type="submit" title="{{ $item->status === 'aktif' ? 'Nonaktifkan' : 'Aktifkan' }}"
                                ...
                                {{-- Hapus (red) --}}
                                <button type="button" title="Hapus Lapak"
                                    @click="deleteId = {{ $item->id }}; deleteName = '{{ addslashes($item->nama_toko) }}'; showDeleteModal = true"
                        ...
                        <td class="px-3 py-3">
                            <p class="font-semibold text-gray-900 dark:text-slate-100 text-sm">
                                <a href="{{ route('admin.lapak.show', $item) }}" class="hover:text-emerald-600 dark:hover:text-emerald-400 hover:underline">
                                    {{ $item->nama_toko }}
                                </a>
                            </p>
                            @if($item->alamat)
                            <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">{{ Str::limit($item->alamat, 45) }}</p>
                            @endif
                        </td>

                        {{-- PEMILIK --}}
                        <td class="px-3 py-3 text-sm text-gray-700 dark:text-slate-300 whitespace-nowrap">
                            {{ $item->penduduk->nama ?? '—' }}
                        </td>

                        {{-- TELEPON --}}
                        <td class="px-3 py-3 text-sm text-gray-500 dark:text-slate-400 hidden lg:table-cell whitespace-nowrap">
                            {{ $item->telepon ?? '—' }}
                        </td>
                        ...
                        <td class="px-3 py-3 text-center">
                            <a href="{{ route('admin.lapak.produk.index', $item) }}"
                                class="inline-flex items-center justify-center w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-full text-sm font-semibold text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors">
                                {{ $item->produk_count }}
                            </a>
                        </td>
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-gray-500 dark:text-slate-400 font-medium">Belum ada data lapak</p>
                                <a href="{{ route('admin.lapak.create') }}"
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between flex-wrap gap-3">
            <p class="text-sm text-gray-500 dark:text-slate-400">
                @if($lapak->total() > 0)
                    Menampilkan {{ $lapak->firstItem() }} sampai {{ $lapak->lastItem() }} dari
                    {{ number_format($lapak->total()) }} entri
                @else
                    Menampilkan 0 sampai 0 dari 0 entri
                @endif
            </p>
            <div class="flex items-center gap-1">
                @if($lapak->onFirstPage())
                ...
                @else
                    <a href="{{ $lapak->appends(request()->query())->previousPageUrl() }}"
                ...
                @for($p = max(1, $cp - 2); $p <= min($lp, $cp + 2); $p++)
                    ...
                    @else
                        <a href="{{ $lapak->appends(request()->query())->url($p) }}"
                ...
                @if($lapak->hasMorePages())
                    <a href="{{ $lapak->appends(request()->query())->nextPageUrl() }}"
    ...
    <div x-show="showDeleteModal"
        ...
        <div @click.outside="showDeleteModal = false"
            ...
            <h3 class="text-base font-bold text-gray-900 dark:text-slate-100 text-center mb-1">Hapus Lapak?</h3>
            <p class="text-sm text-gray-500 dark:text-slate-400 text-center mb-1">
                Lapak <span class="font-semibold text-gray-700 dark:text-slate-200" x-text="deleteName"></span>
            </p>
            <p class="text-sm text-gray-500 dark:text-slate-400 text-center mb-6">
                Semua produk di lapak ini juga akan terhapus permanen.
            </p>
