...
<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 mb-6">
    <form method="GET" action="{{ route('admin.ppid.index') }}" class="flex flex-wrap items-end gap-3">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                @forelse($dokumen as $item)
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        {{-- AKSI: Lihat, Edit, Hapus saja (mirip OpenSID) --}}
                        <td class="px-4 py-4">
                        ...
                        <td class="px-4 py-4 text-sm text-gray-600 dark:text-slate-400 whitespace-nowrap">
                            @if($item->retensi_nilai && $item->retensi_satuan)
                                {{ $item->retensi_nilai }} {{ $item->retensi_satuan }}
                            @elseif($item->waktu_retensi)
                                {{ $item->waktu_retensi }}
                            @else
                                -
                            @endif
